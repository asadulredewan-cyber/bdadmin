...
function renderForm(data = {}) {
  ...
  heroRoot.innerHTML = `
    <div class="edit-box">
      ...
      <div style="display:flex;gap:8px;align-items:center">
        ...
        <button id="pickHeroImg">Select Image</button>
      </div>

      <div style="display:flex;gap:8px;align-items:center;margin-top:8px">
        <input id="h_bg_picker" type="color" value="${bg}">
        <input id="h_bg" type="text" value="${bg}" style="width:110px">
        <span id="bgPreview"
          style="width:32px;height:32px;border:1px solid #ccc;border-radius:6px;background:${bg}">
        </span>
      </div>
  ...
  /* ===== IMAGE PICKER ===== */
  /* ===== IMAGE PICKER (HERO) ===== */
const imgInput = document.getElementById("h_image");

document.getElementById("pickHeroImg").onclick = () =>
  openImagePicker({
    input: imgInput,
    add: url => {
      imgInput.value = url;
  ...
  /* ===== COLOR PICKER SYNC ===== */
  const picker = document.getElementById("h_bg_picker");
  const text   = document.getElementById("h_bg");
  ...
  text.oninput = () => {
    if (/^#([0-9A-F]{3}){1,2}$/i.test(text.value)) {
      ...
      prev.style.background = text.value;
    }
  };
